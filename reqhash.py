...
def get_output(cmd, include_stderr=False, **kwargs):
    print("Getting output from {}".format(cmd), file=sys.stderr)
    try:
...
def print_prod_req(module_dict):
    print(pprint.pformat(module_dict), file=sys.stderr)
    print("# Generated from reqhash.py")
